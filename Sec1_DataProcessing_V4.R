...
# This section is primarily for quality control purposes of DNA barcode data obtained from the BOLD API. Filter arguments may be altered to meet the user's needs.
...
# Find sequences that begin (^) with an N or a gap and split them using tstrsplit. Take only the second half of the split.strsplit gives a blank first element here when splitting at the beginning of the string, so that is why the chained command is neccessary.
...
### FILTER 7 ###
# Remove rows with no species information. This will remove BINs without any species information. BINs without species data would not match with any trait information down the line.
# Create a new datatable containing only sequences baring species-level identification. This is so we can extract the BIN URIs that contain species-level identification and remove BIN URIs without species information. 
...
# These steps are performed to improve BIN reliability and ensure we are matching the appropriate sequence information to the appropriate trait data down the line.
...
# If there are more than 0 conflicts, you may apply the ResolveBIN function to remove a deviant record from a BIN. You may want to do this if there are a few weird records in your BIN, but the BIN seems reliable otherwise.
...
# EXAMPLE: Or you can remove an entire BIN from the dataset. You may want to do this if there is no clear consensus in the BIN for an order level assignment.
...
# There are probably going to be a lot more genus and species level conflicts, so we will not be able to check them manually. Instead, we will keep only those BINs that have AT LEAST 10 records and that have 80% consistency for genus or species level assignment (i.e. at least 8 out of 10 records share the same genus or species level assignment). You can change these thresholds if you want to make them more or less strict.
...
### SPECIES LABEL ###
# Now, we want to assign every sequence in a BIN taxonomic labels. This will ensure that even those sequences with discordant taxonomic classifications will share a common name with the "accepted" taxonomic assignment for their BIN. This is necessary for matching trait information using species names.
...
dfFiltered <- dfFiltered[, .(bin_uri, recordID, order_name, order_label, family_name, family_label, genus_name, genus_label, species_name, species_label, nucleotides, filtered_bin_size, lat)]
