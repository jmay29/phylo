...
keep <- dfSpeciesTraits[, which(type_length == "TL")]
dfMaxLength <- dfSpeciesTraits[keep, ]
# TEST 1: Does the trait have an adequate sample size?
...
# Which columns are integers? I want to add these columns together.
integerVars <- dfSalinity[, lapply(.SD, is.integer)]
...
#rareVars <- which(dfSalinity$salinity == "brackish")
#dfSalinity <- dfSalinity[-rareVars, ]
# Make sure to also drop the level from the factor.
dfSalinity$salinity <- droplevels(dfSalinity$salinity)
...
#rareVars <- which(dfParentalCare$parental_care == "biparental")
#dfParentalCare <- dfParentalCare[-rareVars, ]
# Also dropping these levels from the factor.
dfParentalCare[, parental_care := as.factor(parental_care)]
dfParentalCare$parental_care <- droplevels(dfParentalCare$parental_care)
...
dfMissing <- dfTraits[apply(dfTraits[, c(5:12)], 1, 
