...
extremeSeqs <- foreach(i = 1:nrow(dfCheckCentroidSeqs)) %do% 
...
# Extract the BIN URIs of the extremely gappy sequences.
...
dfCentroidSeqs <- dfCentroidSeqs[!dfCentroidSeqs$bin_uri %in% extremeBins, ]
...
if (nrow(dfNonmatchOrd) > 0) {
  ...
  dfCentroidSeqs <- dfCentroidSeqs[!dfCentroidSeqs$bin_uri %in% nonMatchOrdBins, ]
}
...
# Again, remove non-matching BINs if desired:
if (nrow(dfNonmatchOrd) > 0) {
  nonMatchOrdBins <- unique(dfNonmatchOrd$bin_uri)
  dfCentroidSeqs <- dfCentroidSeqs[!dfCentroidSeqs$bin_uri %in% nonMatchOrdBins, ]
}
...
# To remove:
if (nrow(dfNonmatchFam) > 0) {
  nonMatchFamBins <- unique(dfNonmatchFam$bin_uri)
  dfCentroidSeqs <- dfCentroidSeqs[!dfCentroidSeqs$bin_uri %in% nonMatchFamBins, ]
}
