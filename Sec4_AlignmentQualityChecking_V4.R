...
# Identify BINs with no relatives within "typical" range of genetic divergence (i.e. all of their genetic distances fall outside 1.5 x IQR upper threshold.)
dfOutlierCheck <- dfOutlierCheck[apply(dfOutlierCheck, 1, function(x) all(x > upperThreshold)), ]
# Take a closer look.
outliers <- row.names(dfOutlierCheck)
# If desired, remove the outliers from dfCentroidSeqs as we will be retrimming these sequences again once troublesome cases are removed.
dfCentroidSeqs <- RemoveSequences(dfCentroidSeqs, outliers)

### NEAREST NEIGHBOUR CHECK ###
# Remove centroid sequences whose nearest neighbours are in a different order or family. The nearest neighbour can be determined 
# from the distance matrix. It is the sequence with minimum pairwise distance to the sequence in question.
...
dfNearestNeighbour <- transform(dfNearestNeighbour, V1 = colsplit(V1, split = "/", names = c('bin_uri', 'nearest_neighbour')))
...
# Convert into another dataframe first (we need to do this to fully separate the bin_uri and nearest_neighbour columns).
...
df2NearestNeighbour <- as.data.table(df2NearestNeighbour)
# Make a datatable called dfBinOrd and match the order of the bin_uris in this dataframe to the order of the bin_uris 
# in dfCheckCentroidSeqs so that we can extract the order and family names of the BINs.
...
# The next few lines may be used to remove the sequences, if any are found! (you should double check these cases on the BOLD website, too).
dfCentroidSeqs <- RemoveSequences(dfCentroidSeqs, dfNonmatchOrd$bin_uri)
# Now, let's take a look at the BINs where the nearest neighbour is in a different family.
...
# Now, you can perform a more comprehensive check if desired. Do the BINs have ANY close neighbours within a genetic distance of 0.05 
# that are in  a different order or family? Again, this may be indicative of something weird going on in either the sequence data or 
# taxonomic assignment. First, subset out all close neighbour pairings that share a genetic distance under 0.05 to any other sequence.
...
# Following the same protocol as before, extract the order and family names of the BINs and their close neighbours.
